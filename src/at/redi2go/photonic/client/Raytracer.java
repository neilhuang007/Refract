...
public class Raytracer implements Destructable {
    ...
    public Raytracer() {
        String shaderPackName = Iris.getIrisConfig().getShaderPackName().orElseThrow(() -> new RuntimeException("No shaderpack selected!"));
        this.shaderPackPath = Iris.getShaderpacksDirectory().resolve(shaderPackName);

        ShaderPack buffers = Iris.getCurrentPack().orElseThrow();
        USED_BUFFERS.clear();
        for (int integer : buffers.getBufferObjects().keySet()) {
            USED_BUFFERS.add(integer);
        }
    ...
    public static void filterDoubleFaces(Map<Direction, ModelElementFace> faces, Vector3f from, Vector3f to) {
        ...
        if (delta.x * delta.y * delta.z == 0.0F) {
            ...
            for (Direction direction : Direction.values()) {
                if (!(Vec3d.of(direction.getVector()).dotProduct(new Vec3d(normal.x, normal.y, normal.z))
                ...
                for (Map.Entry<Direction, ModelElementFace> entry : faces.entrySet()) {
                    ...
                    ModelElementFace value = entry.getValue();
                    Direction oppositeDirection = key.getOpposite();
                    if (faces.containsKey(oppositeDirection) && key == direction) {
    ...
    public static void reloadPatches() {
        ...
        try (Stream<Path> fsDevStream = Files.exists(fsDevPath) ? Files.list(fsDevPath) : Stream.of();
             Stream<Path> fsStream = Files.exists(fsPath) ? Files.list(fsPath) : Stream.of();
             Stream<Path> classStream = loadClassStream()) {
            Stream.of(fsStream, fsDevStream, classStream)
                    .flatMap(s -> s)
                    .filter(Files::exists)
                    .map(p -> {
    ...
    public static @Nullable String readShaderFile(ShaderPackPath path, boolean patchFile) {
        String source = null;
