...
public class ReferenceWorldBackend extends LegacyWorldBackend {
   private ReferenceUploadSummary lastUploadSummary = ReferenceUploadSummary.empty();

   public ReferenceWorldBackend(int worldChunkSize, Map<PChunkPos, WorldChunk> chunks) {
      super(worldChunkSize, chunks);
   }

   @Override
   public boolean usesNativeBlockPayloads() {
      return false;
   }
   ...
   public void updateRootData(PChunkPos rtToWorldChunkOffset, int worldChunkSize, boolean fullRootRebuild) {
      ...
      if (rootMemory == null) {
         this.lastUploadSummary = ReferenceUploadSummary.empty();
         return;
      }
   ...
   public ReferenceUploadSummary consumeUploadSummary() {
      ReferenceUploadSummary summary = this.lastUploadSummary;
      this.lastUploadSummary = ReferenceUploadSummary.empty();
      return summary;
   }

   public ReferenceUploadSummary buildCurrentSummary() {
      int populatedRootEntries = this.countLoadedRootEntries();
      return this.buildUploadSummary(populatedRootEntries);
   }

   private ReferenceUploadSummary buildUploadSummary(int populatedRootEntries) {
      int dirtyChunks = 0;
      int dirtyVoxels = 0;
      for (WorldChunk chunk : this.getChunks().values()) {
         if (!(chunk instanceof ReferenceChunk referenceChunk)) {
            continue;
         }
         if (!referenceChunk.isDirty()) {
            continue;
         }
         dirtyChunks++;
         dirtyVoxels += referenceChunk.getDirtyVoxelWrites();
      }

      int chunkUploadOps = this.getCbMemoryManager().getPendingUploadCount();
      long chunkUploadBytes = (long) chunkUploadOps * ReferenceChunk.byteSize;
      ...
      return new ReferenceUploadSummary(dirtyChunks, dirtyVoxels, chunkUploadOps, chunkUploadBytes, populatedRootEntries, rootUploadBytes);
   }
   ...
   public static final class ReferenceUploadSummary {
      ...
      private ReferenceUploadSummary(int dirtyChunks, int dirtyVoxels, int chunkUploadOps, long chunkUploadBytes, int populatedRootEntries, long rootUploadBytes) {
      ...
      private static ReferenceUploadSummary empty() {
         return new ReferenceUploadSummary(0, 0, 0, 0L, 0, 0L);
      }
