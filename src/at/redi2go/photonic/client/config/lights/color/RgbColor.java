...
public record RgbColor(int packedColor) implements LightColor {
   ...
   public static RgbColor fromHex(String hexString) {
      if (hexString.length() != 7) throw new IllegalArgumentException("must be of length 7");
      if (hexString.charAt(0) != '#') throw new IllegalArgumentException("must start with #");
      return new RgbColor(Integer.parseInt(hexString.substring(1), 16));
   }
   ...
   public static RgbColor fromRgbString(String rgbString) throws CommandSyntaxException {
      ...
      if (r < 0.0F || r > 1.0F) throw new IllegalArgumentException("Expected float between 0 and 1, found: " + r);
      reader.expect(',');
      reader.skipWhitespace();
      float g = reader.readFloat();
      if (g < 0.0F || g > 1.0F) throw new IllegalArgumentException("Expected float between 0 and 1, found: " + g);
      reader.expect(',');
      reader.skipWhitespace();
      float b = reader.readFloat();
      if (b < 0.0F || b > 1.0F) throw new IllegalArgumentException("Expected float between 0 and 1, found: " + b);
      reader.skipWhitespace();
      reader.expect(')');
