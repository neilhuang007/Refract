...
public class BlockEntryBuilderImpl extends RegionMapping implements BlockEntry.Builder {
    ...
    public @Nullable Builder clearRegions(ShortSet regions) {
        ...
        var builder = new BlockEntryBuilderImpl(allocator);
        builder.skylight = skylight;

        for (int voxelIndex = 0; voxelIndex < RtVoxel.ENTRIES_SIZE; voxelIndex++) {
            var entry = data[voxelIndex];
            if (entry == null) continue;
            if (regions.contains(getRegion(voxelIndex))) continue;

            builder.data[voxelIndex] = new MutablePaletteEntry(entry);
            builder.setRegion(voxelIndex, getRegion(voxelIndex));
        }

        return builder.hasAnyVoxel() ? builder : null;
    }
    ...
    public BlockEntry build() {
        ...
        var shift = IntPacking.shiftFactor(Math.max(2, palette.size() << 1));
