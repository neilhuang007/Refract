...
// ph_debug_view_mode is declared in samplers.glsl (included via reuse_bridge.glsl)
...
void main() {
    ...
    // Debug visualization — runs before checkerboard so ALL pixels get filled
    if (ph_debug_view_mode > 0.5) {
        RAB_Surface surface = RAB_GetGBufferSurface(pixelPosition, false);
        if (!RAB_IsSurfaceValid(surface)) {
            direct_diffuse_frag_out = vec4(0.0, 0.0, 0.0, 1.0);
            direct_specular_frag_out = vec4(0.0);
            return;
        }
        int debugMode = int(ph_debug_view_mode + 0.5);
        vec3 debugColor = vec3(1.0, 0.0, 1.0); // magenta fallback

        if (debugMode == 1) {
            float dist = length(surface.worldPos - ph_regir_grid_center);
            float t = clamp(dist / 256.0, 0.0, 1.0);
            debugColor = lt_debug_heat_ramp(t);
        } else if (debugMode == 2) {
            ivec3 cellCoord;
            bool inside = regir_world_to_cell(surface.worldPos, cellCoord);
            debugColor = inside ? vec3(0.0, 1.0, 0.0) : vec3(1.0, 0.0, 0.0);
        } else if (debugMode == 3) {
            ivec3 cellCoord;
            bool inside = regir_world_to_cell(surface.worldPos, cellCoord);
            if (inside) {
                debugColor = vec3(cellCoord) / vec3(ph_regir_grid_cells);
            } else {
                debugColor = vec3(0.0);
            }
        } else if (debugMode == 4) {
        ...
        direct_diffuse_frag_out = vec4(debugColor, 1.0);
        direct_specular_frag_out = vec4(0.0);
        return;
    }
