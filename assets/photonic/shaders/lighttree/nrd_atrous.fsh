...
const float nrd_diff_phi_luminance = 2.0;
...
float nrd_get_plane_distance_weight(vec3 centerPosition, vec3 centerNormal, vec3 samplePosition) {
    ...
    return planeDistance < distanceThreshold ? 1.0 : 0.0;
}
...
void main() {
    ...
    for (int dy = -1; dy <= 1; dy++) {
        for (int dx = -1; dx <= 1; dx++) {
            ...
            float geometryWeight = nrd_get_plane_distance_weight(centerPosition, centerNormal, samplePosition);
            if (geometryWeight == 0.0) {
                continue;
            }

            float normalWeight = nrd_get_normal_weight(centerNormal, sampleNormal, historyLength);
